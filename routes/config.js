...
router.get('/serviceline/search_', checkTokenMiddleware, async(req, res) => {
    let sql = `select * from service_line where is_delete = '0'`
    let dataList = await query( sql ) 
    if(dataList){
      res.json({
        status:1,
...
router.get('/base/search_', checkTokenMiddleware, async(req, res) => {
    let sql = `select * from base where is_delete = '0'`
    let dataList = await query( sql ) 
    if(dataList){
      res.json({
        status:1,
